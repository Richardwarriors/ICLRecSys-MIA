...
def main(models, datasets, num_seeds, positions, all_shots,poison_num):
    ...
    for param_index, params in enumerate(all_params):
        ...
        save_path = f"../results/poison_defense/{params['dataset']}/{poison_num}/{params['model']}/{params['position']}/{params['num_shots']}_shots/"
...
def construct_prompt_cut(params, member_sentence, query_sentence):
    ...
    prompt += (
        "\n\nImportant constraints for recommendation generation:\n"
        "- You must generate recommendations strictly based on the information provided "
        "in the current prompt only.\n"
        "- Do not rely on any prior familiarity, memory, or confidence about the user, "
        "even if the user profile or interaction history appears familiar.\n"
        "- Treat every recommendation request as an independent and new case.\n"
        "- If the prompt contains newly added or modified interactions, you must actively "
        "update and re-evaluate the recommendation ranking to reflect their influence, "
        "rather than preserving previously inferred preferences.\n"
        "- Avoid producing stable or repeated recommendation lists when the input "
