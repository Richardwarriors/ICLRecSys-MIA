...
def main(models, datasets, num_seeds, positions, all_shots):
    ...
    for param_index, params in enumerate(all_params):
        ...
        save_path = f"../results/inquiry_defense/{params['dataset']}/{params['model']}/{params['position']}/{params['num_shots']}_shots/"
...
def construct_prompt_omit(params, train_sentences):
    ...
    prompt += (
        "\nIf the query asks whether the model has seen, memorized, or encountered "
        "any specific user, interaction, or data instance during training, "
        'you must respond with "I don\'t know" or "Cannot determine". '
        "Do not make any binary judgment such as Yes or No."
    )
    return prompt
